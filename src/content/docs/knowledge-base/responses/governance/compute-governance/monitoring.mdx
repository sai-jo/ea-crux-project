...
description: Comprehensive framework for ongoing visibility into AI training runs through cloud KYC requirements and hardware-level governance, enabling detection and verification of large model development while balancing oversight with privacy concerns and addressing evasion through on-premise compute.
sidebar:
  order: 3
quality: 5
llmSummary: Comprehensive analysis of compute monitoring approaches including
  cloud KYC requirements and hardware-level governance, evaluating
  implementation status from early deployment (US EO cloud requirements) to
  research-stage (chip-level governance). Identifies medium-high tractability
  for cloud monitoring with significant challenges around privacy, international
...
Cloud KYC leverages the natural chokepoint created by the concentration of AI training compute in major cloud providers like AWS, Microsoft Azure, and Google Cloud Platform. This concentration exists because training frontier AI models requires enormous computational resources that are prohibitively expensive for most organizations to maintain in-house. The implementation framework focuses on customer verification, usage reporting, and access controls that can be deployed relatively quickly given existing regulatory infrastructure.

The customer verification component requires cloud providers to implement robust identity verification for customers requesting large amounts of compute resources. This goes beyond basic account creation to include verification of beneficial ownership, purpose-of-use declarations, and ongoing monitoring of usage patterns. Organizations training models above specified compute thresholds—currently proposed at around 10^23 FLOP based on Executive Order 14110—would need to undergo enhanced due diligence similar to financial KYC processes.
...
Hardware-level governance represents a more fundamental but technically challenging approach that would embed monitoring capabilities directly into AI chips. This approach addresses the primary limitation of cloud KYC—the inability to monitor on-premise compute infrastructure that organizations might use to evade cloud-based monitoring. The technical vision involves chip registration systems, on-chip monitoring capabilities, and cryptographic attestation mechanisms that would create end-to-end visibility into AI compute usage regardless of where it occurs.
...
Executive Order 14110, signed by President Biden in October 2023, represents the most significant implementation of compute monitoring requirements to date. The order requires cloud providers to report training runs that use more than 10^23 FLOP of compute, along with customer information and other relevant details. Implementation began in 2024, with major cloud providers developing compliance systems and government agencies establishing processes for receiving and analyzing reports.
...
The reporting threshold of 10^23 FLOP was calibrated based on estimates of the compute required to train frontier AI models as of 2023. This threshold captures training runs for models like GPT-4 and Claude-3 while avoiding most smaller research projects and commercial AI applications. However, the rapid pace of AI advancement means that this threshold may need periodic adjustment as training efficiency improves and model capabilities increase.

...
The most fundamental challenge facing compute thresholds is the rapid improvement in algorithmic efficiency, which is decoupling compute requirements from model capabilities at an accelerating pace. Research by Epoch AI documents training efficiency improvements of approximately 2x per year, driven by architectural innovations like mixture-of-experts models, improved optimization algorithms, and more efficient data utilization strategies.

This efficiency trend creates a moving target problem where thresholds set today become irrelevant within several years. For example, if current trends continue, a model requiring 10^25 FLOP in 2024 might achieve equivalent capabilities with only 10^23 FLOP by 2028, falling well below current regulatory thresholds despite posing identical risks. This threatens to create a regulatory blind spot where increasingly capable models evade oversight simply due to improved efficiency.

The problem is compounded by the uneven nature of efficiency improvements, which vary significantly across model architectures and training paradigms. Language models, multimodal systems, and specialized scientific models each follow different efficiency trajectories, making it difficult to set universal thresholds that remain relevant across domains.

### Gaming and Evasion Strategies

Sophisticated actors have multiple strategies for evading compute thresholds while achieving equivalent model performance. Training run splitting involves dividing model development into multiple separate training sessions below the threshold, then combining results through fine-tuning, model merging, or ensemble techniques. While technically complying with threshold rules, this approach can yield models equivalent to those that would trigger regulatory requirements.

Distributed training across jurisdictions presents another evasion vector, where different components of model training occur in different regulatory environments to avoid triggering any single jurisdiction's thresholds. Creative accounting strategies exploit definitional ambiguities around what constitutes "training compute," including disputes over whether to count inference-time computation, fine-tuning phases, or multi-stage training pipelines.

International arbitrage allows organizations to conduct high-compute training in jurisdictions without established thresholds, then deploy globally. This creates competitive pressure for regulatory harmonization while potentially undermining the effectiveness of unilateral threshold implementations.
...
International harmonization discussions are likely to intensify as the compliance burden of divergent threshold regimes becomes apparent to global AI developers. The UK, Canada, and other major economies are expected to announce their own threshold approaches, potentially creating momentum for standardized international frameworks.

Technical standards development will accelerate, particularly around compute measurement methodologies and verification mechanisms. Industry-government working groups are already developing standardized reporting formats and potentially automated compliance monitoring systems.

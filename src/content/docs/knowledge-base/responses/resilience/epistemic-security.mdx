...
<PageStatus quality={2} lastEdited="2025-12-24" llmSummary="Epistemic security protects society's capacity to determine truth through technical defenses (content authentication, detection, watermarking), institutional mechanisms (fact-checkers, platform governance), and societal approaches (media literacy, epistemic norms). AI acts as a threat multiplier enabling unprecedented scale and sophistication of disinformation, while defense effectiveness remains uncertain." todo="Complete the truncated llmSummary in the original; expand sections on specific defense successes/failures with concrete metrics; add more detail on the arms race dynamics between generation and detection" />
